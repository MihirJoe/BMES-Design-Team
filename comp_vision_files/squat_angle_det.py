...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            angle_ankle = calculate_angle(knee, ankle, hip)  # Ankle joint angle
            ankle_angle = 180 - angle_ankle

            # Exclusive RGB values
            ankle_color = (255, 0, 255)  # Magenta
            cv2.putText(image, str(angle_ankle),tuple(np.multiply(ankle, [640, 480]).astype(int)), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, ankle_color, 2, cv2.LINE_AA)
